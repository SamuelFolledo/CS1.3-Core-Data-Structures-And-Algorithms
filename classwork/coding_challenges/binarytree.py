# https://github.com/Make-School-Courses/CS-1.3-Core-Data-Structures/blob/master/Code/trees/binarytree.py
#!python
...
class BinaryTreeNode(object):
    ...
    def height(self):
        ...
        if self.is_leaf(): #ensure it is not leaf
            return 0
        return self.get_height_recursively(self)

    def get_height_recursively(self, node, height = 0):
        if node == None: #if None, take back the height+1
            return height - 1
        left_height = self.get_height_recursively(node.left, height+1) #recursively increment height
        right_height = self.get_height_recursively(node.right, height+1)
        return max(left_height, right_height) #return whichever from left or right height is greater
...
class BinarySearchTree(object):
    ...
    def insert(self, item):
        ...
        if parent == None: #parent is None if tree's size is 1, then parent will be the root
            parent = self.root
    ...
    def delete(self, item):
        ...
        # print(f"Deleting {item} {node} {parent}")
        is_left_of_parent = parent.left is node
        # print("\n\nIS Left?", is_left_of_parent)
